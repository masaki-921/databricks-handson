...
| `TS_COL` | `event_time` | タイムスタンプ列名（両テーブル共通） |
| `MSG_COL` | `message` | 本文列名（両テーブル共通） |
| `LEVEL_COL` | `level` | **silver_app** のエラー判定列（`== 'ERROR'`） |
| `ORA_CODE_COL` | `ora_code` | **silver_oracle** のエラー判定列（ORAコードが入っている行をエラーとみなす） |
| `WINDOW_MIN` | `10` | 相関ウィンドウ（前後の分数） |
| `AI_MODEL` | `gpt-5-mini` | 利用モデル。代替: `gpt-5.1` / `gpt-4.1` |

> **エラー判定の仕様:** silver_app は `level == 'ERROR'`、silver_oracle は `ora_code` が非NULL（空文字でない）行をエラーとして扱います。出力にはOracleの `ora_code` が `[ORA-xxxxx]` 形式で併記されます。
...
| 列が見つからない | CELL 2 の `TS_COL` / `MSG_COL` / `LEVEL_COL` / `ORA_CODE_COL` を実際の列名に合わせる |
| Oracleが0件ばかり | `silver_oracle` の `ora_code` 列に値が入っているか確認。列名が異なる場合は `ORA_CODE_COL` を変更 |
